--[[
  EngineRunner integration tests.
  Runs all registered engines on various board positions via EngineRunner, asserts
  the generated move is legal, and uses Test.assertSnapshot for consistency (one file per assertion).
  First run (or new test cases) creates snapshot files in __snapshot__/.
  To overwrite existing snapshots: run with UPDATE_SNAPSHOTS=1.
]]
...
-- Resolve __snapshot__ directory for engine_runner snapshots (one file per assertion)
...
-- Run engine on FEN and return (result, err). Uses low limits for speed.
local function runEngineSync(engineId, fen, opts)
  opts = opts or {}
  local result, err
  ...
  if opts.ply_limit then
    builder:PlyLimit(opts.ply_limit)
  end
  if opts.node_limit then
    builder:NodeLimit(opts.node_limit)
  end
  -- Do not pass time_limit so results are predictable (ply/node limited only)
  builder:Run()
  return result, err
end

-- Run engine, assert legal move, then Test.assertSnapshot(name, move). Returns the move.
local function runEngineAndAssertSnapshot(engineId, fen, opts, snapshotName, msg)
  msg = msg or ("engine " .. engineId)
  local result, err = runEngineSync(engineId, fen, opts)
  Test.assertNil(err, msg .. " (error: " .. tostring(err and (err.message or err) or "") .. ")")
  Test.assertNotNil(result, msg .. " (nil result)")
  Test.assertNotNil(result.move, msg .. " (no result.move)")
  Test.assertTrue(MoveGen.IsLegalMove(fen, result.move),
    msg .. " (illegal move: " .. tostring(result.move) .. ")")
  Test.assertSnapshot(snapshotDir, snapshotName, result.move)
  return result.move
end

-- Parse EPD line to { fen, name, bm }. EPD: position fields then opcodes (e.g. bm Rxf4; id "...").
-- FEN part ends at first ";"; ensure 6 fields (add " 0 1" or " 1" if missing). Name from id opcode or FEN.
-- bm: optional, list of SAN best moves (single move or "bm m1 m2 ...").
local function parseEPD(epdString)
  epdString = epdString:gsub("^%s+", ""):gsub("%s+$", "")
  local fenPart = epdString
  local firstSemi = epdString:find(";")
  if firstSemi then
    fenPart = epdString:sub(1, firstSemi - 1):gsub("%s+$", "")
  end
  -- Extract space-separated fields from fenPart
  local fields = {}
  for f in fenPart:gmatch("%S+") do fields[#fields + 1] = f end
  -- FEN has at most 6 fields; extra tokens are EPD opcodes (e.g. "bm Rf1").
  -- Fields 5 & 6 (halfmove clock, fullmove number) must be numeric.
  local fenFieldCount = 4 -- minimum: position, side, castling, ep
  if #fields >= 5 and fields[5]:match("^%d+$") then
    fenFieldCount = 5
    if #fields >= 6 and fields[6]:match("^%d+$") then
      fenFieldCount = 6
    end
  end
  local fen = table.concat(fields, " ", 1, math.min(fenFieldCount, #fields))
  if fenFieldCount <= 4 then
    fen = fen .. " 0 1"
  elseif fenFieldCount == 5 then
    fen = fen .. " 1"
  end
  local name = fen
  local idVal = epdString:match('id "%s*([^"]*)"')
  if idVal and idVal ~= "" then
    name = idVal
  end
  local bmList = nil
  local bmStr = epdString:match("bm%s+([^;]+)")
  if bmStr then
    bmList = {}
    for san in bmStr:gmatch("%S+") do
      bmList[#bmList + 1] = san:gsub("[%+#]$", "")
    end
  end
  return { fen = fen, name = name, bm = bmList }
end

-- Return true if engineMove (UCI) equals one of the best moves (SAN converted to UCI). Info only, no assert.
local function engineFoundBm(fen, bmList, engineMoveUci)
  if not bmList or #bmList == 0 or not engineMoveUci or not MoveGen then return true end
  local pos = MoveGen.ParseFen(fen)
  if not pos then return true end
  for _, san in ipairs(bmList) do
    local uci = MoveGen.SanToUci(pos, san)
    if uci and uci == engineMoveUci then return true end
  end
  return false
end

-- Board situations in EPD format (position + optional opcodes like bm, id).
local epdPositions = {
    'rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1; id "DELTA 001 - starting position (white)";',
    'rnbqkbnr/pppp1ppp/8/4p3/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 2; id "DELTA 002 - after e4 e5 (black to move)";',
    ...
    -- '4k3/8/4p3/4K3/8/8/8/8 b - - 0 1; id "DELTA 006 - black to move, simplified";', -- TODO for some reason fruit 2.1 hangs here
    ...
    '4k3/3q4/8/8/4N3/8/8/4K3 w - - 0 1; id "DELTA 010 - knight fork to win queen (white)";',
    ...
    'r3qb1k/1b4p1/p2pr2p/3n4/Pnp1N1N1/6RP/1B3PP1/1B1QR1K1 w - - bm Nxh6; id "TNS 001";',
    'r4rk1/pp1n1p1p/1nqP2p1/2b1P1B1/4NQ2/1B3P2/PP2K2P/2R5 w - - bm Rxc5; id "TNS 002";',
    'r2qk2r/ppp1b1pp/2n1p3/3pP1n1/3P2b1/2PB1NN1/PP4PP/R1BQK2R w KQkq - bm Nxg5; id "TNS 003";',
    'r1b1kb1r/1p1n1ppp/p2ppn2/6BB/2qNP3/2N5/PPP2PPP/R2Q1RK1 w kq - bm Nxe6; id "TNS 004";',
    'r2qrb1k/1p1b2p1/p2ppn1p/8/3NP3/1BN5/PPP3QP/1K3RR1 w - - bm e5; id "TNS 005";',
    'rnbqk2r/1p3ppp/p7/1NpPp3/QPP1P1n1/P4N2/4KbPP/R1B2B1R b kq - bm axb5; id "TNS 006";',
    '1r1bk2r/2R2ppp/p3p3/1b2P2q/4QP2/4N3/1B4PP/3R2K1 w k - bm Rxd8+; id "TNS 007";',
    'r3rbk1/ppq2ppp/2b1pB2/8/6Q1/1P1B3P/P1P2PP1/R2R2K1 w - - bm Bxh7+; id "TNS 008";',
    'r4r1k/4bppb/2n1p2p/p1n1P3/1p1p1BNP/3P1NP1/qP2QPB1/2RR2K1 w - - bm Ng5; id "TNS 009";',
    'r1b2rk1/1p1nbppp/pq1p4/3B4/P2NP3/2N1p3/1PP3PP/R2Q1R1K w - - bm Rxf7; id "TNS 010";',
    'r1b3k1/p2p1nP1/2pqr1Rp/1p2p2P/2B1PnQ1/1P6/P1PP4/1K4R1 w - - bm Rxh6; id "TNS 011";',
...
local positions = {}
for _, epd in ipairs(epdPositions) do
  table.insert(positions, parseEPD(epd))
end
...
for _, engine in ipairs(engineList) do
  local engineId, displayName = engine.id, (engine.name or engine.id)
  for _, pos in ipairs(positions) do
    local posName = pos.name or pos.fen
    local snapshotName = engineId .. "|" .. posName
    Test.test(displayName .. ": " .. posName, function()
      local move = runEngineAndAssertSnapshot(engineId, pos.fen, defaultOpts, snapshotName)
      if pos.bm and not engineFoundBm(pos.fen, pos.bm, move) then
        local bmStr = table.concat(pos.bm, " or ")
        local playedStr = tostring(move)
        if MoveGen then
          local posObj = MoveGen.ParseFen(pos.fen)
          if posObj then
            local san = MoveGen.UciToSan(posObj, move)
            if san then playedStr = san end
          end
        end
        print("      [info] expected bm " .. bmStr .. ", engine played " .. playedStr)
      end
    end)
  end
end
...
--------------------------------------------------------------------------------
-- Each engine: starting position with higher limits
--------------------------------------------------------------------------------

for _, engine in ipairs(engineList) do
  local engineId, displayName = engine.id, (engine.name or engine.id)
  local snapshotName = engineId .. "|limits|starting position"
  Test.test(displayName .. ": starting position with limits", function()
    runEngineAndAssertSnapshot(engineId, positions[1].fen, { ply_limit = 3, node_limit = 2000 }, snapshotName)
  end)
end

--------------------------------------------------------------------------------
-- Each engine: same position (starting)
--------------------------------------------------------------------------------

for _, engine in ipairs(engineList) do
  local engineId, displayName = engine.id, (engine.name or engine.id)
  Test.test(displayName .. ": same position", function()
    runEngineAndAssertSnapshot(engineId, positions[1].fen, { ply_limit = 2, node_limit = 800 }, engineId .. "|same position")
  end)
end

--------------------------------------------------------------------------------
-- Each engine: black to move
--------------------------------------------------------------------------------

for _, engine in ipairs(engineList) do
  local engineId, displayName = engine.id, (engine.name or engine.id)
  Test.test(displayName .. ": black to move", function()
    runEngineAndAssertSnapshot(engineId, positions[2].fen, { ply_limit = 2, node_limit = 800 }, engineId .. "|black to move")
  end)
end

if #engineList == 0 then
  Test.test("No engines registered (placeholder)", function()
    Test.assertTrue(true, "Register engines (e.g. GarboChess, Sunfish) to run engine integration tests")
  end)
end