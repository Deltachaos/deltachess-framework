...
--------------------------------------------------------------------------------
-- Game status (application-level: GetGameStatus, StartGame, Pause, etc.)
--------------------------------------------------------------------------------

Test.test("GetGameStatus: nil when no start time", function()
  local board = Board.New()
  Test.assertNil(board:GetGameStatus(), "no start time")
end)

Test.test("StartGame, GetGameStatus, EndGame", function()
  local board = Board.New()
  board:StartGame(1000)
  Test.assertEq(board:GetGameStatus(), Constants.STATUS_ACTIVE, "active after start")
  Test.assertNotNil(board:GetStartTime(), "start time set")
  board:EndGame(2000)
  Test.assertEq(board:GetGameStatus(), Constants.STATUS_ENDED, "ended")
...
1. e4 e5 2. Qh5 Nc6 3. Bc4 Nf6 4. Qxf7
  ...
  Test.assertNil(err, "no error")
  Test.assertEq(board:GetGameStatus(), Constants.STATUS_ENDED, "game ended")
